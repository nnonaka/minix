...
static u32_t dev_gcr_read(vir_bytes base, u32_t reg) {
	u32_t res;
...
static void dev_gcr_write(vir_bytes base, u32_t reg, u32_t val) {
	sdr_out8(base, REG_GCR_INDEX, reg);
	sdr_out32(base, REG_GCR_DATA, val);
}

static void dev_command(vir_bytes base, u32_t cmd) {
	int i;
	for (i = 0; i < 1000; i++) {
